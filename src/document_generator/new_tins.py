...
class NewTinsDocumentGenerator(BaseDocumentGenerator):
    ...
    def _generate_one_sample(self):
        ...
        draw.text(
            annotations["tins_number"]["bboxes"][:2], annotations["tins_number"]["value"], font=self._font, fill="black"
        )
        draw.text(annotations["name"]["bboxes"][:2], annotations["name"]["value"], font=self._font, fill="black")
        draw.text(
            annotations["birth_date"]["bboxes"][:2], annotations["birth_date"]["value"], font=self._font, fill="black"
        )
        draw.text(
            annotations["reg_date"]["bboxes"][:2], annotations["reg_date"]["value"], font=self._font, fill="black"
        )
        draw.text(
