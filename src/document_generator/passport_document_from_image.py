...
class PassportDocumentFromImageGenerator(BaseDocumentGenerator):
    ...
    @staticmethod
    def _put_photo(
        self, pasp_image: Image, photo_path: str, left: int = 220, upper: int = 2590, right: int = 995, down: int = 3580
    ) -> Image:
        """
        ...
        pasp_image.paste(photo.resize((right - left, down - upper)), (left, upper))  # изменяем размер фото
