...
class PassportDocumentDataGenerator(BaseDocumentDataGenerator):
    ...
    def generate() -> PassportData:
        ...
        for ann in anno:
            if ann["value"].get("rectanglelabels") and ann["value"]["rectanglelabels"][0] in pass_data.__dict__.keys():
                 key = ann["value"]["rectanglelabels"][0]
                 x = ann["value"]["x"] / 100 * ann["original_width"]
                 y = ann["value"]["y"] / 100 * ann["original_height"]
                 pass_data.set_bboxes(key, [x, y])
        """
        # print("nomer_podr")
        # print(nomer_podr)
        return PassportData(
