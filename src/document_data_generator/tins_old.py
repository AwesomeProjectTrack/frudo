...
class OldTinsDocumentDataGenerator(BaseDocumentDataGenerator):
    ...
    def generate() -> OldTinsData:
        ...
        start_coords_old = StartCoords.generate(width=anno['old']['width'], 
                                            height=anno['old']['heigth'], is_new_tins=False)
        
        anno['old']['name']['value'] = full_name
        anno['old']['name']['x'] = start_coords_old['name'][0]
        anno['old']['name']['y'] = start_coords_old['name'][1]
        anno['old']['name']['bbox'] =  BboxTins().generate(text= full_name, 
                                                                    old_coords= [anno['old']['name']['x'], 
                                                                                  anno['old']['name']['y']],
                                                                    is_new_tin=False)        
        ...
        anno['old']['birth_date']['bbox'] = BboxTins().generate(text=birth_date, 
                                                                       old_coords= [anno['old']['birth_date']['x'], 
                                                                                  anno['old']['birth_date']['y']], 
                                                                       is_new_tin=False)
        
        anno['old']['sex']['value'] = short_gender
        anno['old']['sex']['x'] = start_coords_old['sex'][0]
        anno['old']['sex']['y'] = start_coords_old['sex'][1]
        anno['old']['sex']['bbox'] = BboxTins().generate(text=gender, 
                                                               old_coords= [anno['old']['sex']['x'], 
                                                                                  anno['old']['sex']['y']], 
                                                               is_new_tin=False)
        ...
        anno['old']['place_of_birth']['bbox'] = BboxTins().generate(text=full_place, 
                                                                           old_coords= [anno['old']['place_of_birth']['x'], 
                                                                                anno['old']['place_of_birth']['y']],
                                                                           is_new_tin=False)
        ...
        anno['old']['issued']['bbox'] = BboxTins().generate(text=registration_date, 
                                                                   old_coords= [anno['old']['issued']['x'], 
                                                                                  anno['old']['issued']['y']],
                                                                   is_new_tin=False)
        ...
        anno['old']['tin']['bbox'] = BboxTins().generate(text=tins_number, 
                                                                   old_coords= [anno['old']['tin']['x'], 
                                                                                  anno['old']['tin']['y']],
                                                                   is_new_tin=False)
