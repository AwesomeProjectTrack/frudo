...
class NewTinsDocumentDataGenerator(BaseDocumentDataGenerator):
    ...
    def generate() -> NewTinsData:
        ...
        start_coords_new = StartCoords.generate(width=anno["new"]["width"], height=anno["new"]["heigth"])

        anno["new"]["name"]["value"] = full_name
        anno["new"]["name"]["x"] = start_coords_new["name"][0]
        anno["new"]["name"]["y"] = start_coords_new["name"][1]
        anno["new"]["name"]["bbox"] = BboxTins().generate(
            text=full_name, old_coords=[anno["new"]["name"]["x"], anno["new"]["name"]["y"]]
        )
        ...
        anno["new"]["birth_date"]["bbox"] = BboxTins().generate(
            text=birth_date, old_coords=[anno["new"]["birth_date"]["x"], anno["new"]["birth_date"]["y"]]
        )

        anno["new"]["sex"]["value"] = gender
        anno["new"]["sex"]["x"] = start_coords_new["sex"][0]
        anno["new"]["sex"]["y"] = start_coords_new["sex"][1]
        anno["new"]["sex"]["bbox"] = BboxTins().generate(
            text=gender, old_coords=[anno["new"]["sex"]["x"], anno["new"]["sex"]["y"]]
        )
        ...
        anno["new"]["place_of_birth"]["bbox"] = BboxTins().generate(
            text=full_place, old_coords=[anno["new"]["place_of_birth"]["x"], anno["new"]["place_of_birth"]["y"]]
        )
        ...
        anno["new"]["issued"]["bbox"] = BboxTins().generate(
            text=registration_date, old_coords=[anno["new"]["issued"]["x"], anno["new"]["issued"]["y"]]
        )

        anno["new"]["tin"]["value"] = tins_number
        anno["new"]["tin"]["x"] = start_coords_new["tin"][0]
        anno["new"]["tin"]["y"] = start_coords_new["tin"][1]
        anno["new"]["tin"]["bbox"] = BboxTins().generate(
            text=tins_number, old_coords=[anno["new"]["tin"]["x"], anno["new"]["tin"]["y"]]
        )
        return NewTinsData(
